...
declare	@sMessage		TMessage
	...
	,@sExec			varchar ( 8000 )=	''	-- 8000 из-за xp_cmdshell
	,@sExec2		varchar ( max )=	''
...
----------
		set	@sExec=	case	@sSubsystem
					...
					when	'ActiveScripting'	then	'cscript /nologo '+	@sFileName		-- cscript для поддержки wscript.stdout.write
				end
	end
	else
		set	@sExec=	@sCommand
...
----------
		if		@sFileName	is	not	null
			and	@sSubsystem	is	not	null	-- используем из процедуры только создание файла без выполнения, поэтому оставляем файл
		begin
